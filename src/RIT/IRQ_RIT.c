...
void RIT_IRQHandler(void) {
    ...
    if (!is_single_board && is_game_started && ((is_player1_turn && this_board_player_id == PLAYER2) || (!is_player1_turn && this_board_player_id == PLAYER1))) return;
    ...
    if ((LPC_GPIO1->FIOPIN & (1 << 28)) == 0) { /* Joystick Right pressed */
        if (!is_double_movement_mode) {
            if (j_right == 1) JOYRIGHT_Callback();
            j_right++;
        }
    }
    else { j_right = 0; }
