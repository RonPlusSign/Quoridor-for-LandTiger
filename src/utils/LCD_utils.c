 ...
 */
void LCD_draw_square(int x0, int y0) { LCD_draw_rect(x0, y0, SQUARE_LENGTH, SQUARE_LENGTH); }
...
/** Initialize screen and draw the initial grid */
...
/**
 * Get pixel coordinates from matrix indexes of the square (top left)
 *
 * @param i i index of the 13x13 matrix
 * @param j j index of the 13x13 matrix
 * @return Point2D with the coordinates of the top left point of the square
 */
...
/**
 *  Inserts a gamer in the grid
 * @param i i index of the 7x7 matrix
 * @param j j index of the 7x7 matrix
 * @param is_player_1 Whether the gamer is player1 or player2
 */
...
/**
 * Draw a wall on the screen at the given coordinates.
 * @param i i index of the 13x13 matrix
 * @param j j index of the 13x13 matrix
...
void LCD_draw_pawn(int x0, int y0, uint16_t color) {
    ...
    char pawn_offset_length[27][2] = {{27, 0}, {27, 0}, {12, 3}, {11, 5}, {10, 7}, {9, 9}, {9, 9},  {9, 9},  {10, 7}, {11, 5}, {8, 11}, {8, 11}, {8, 11}, {10, 7},
                                      {10, 7}, {10, 7}, {10, 7}, {10, 7}, {10, 7}, {9, 9}, {8, 11}, {8, 11}, {6, 15}, {6, 15}, {6, 15}, {6, 15}, {27, 0}};
...
/**
 * Draws the given message between the grid and the scoreboard
...
void LCD_draw_message(uint8_t *msg) {
    ...
    GUI_Text(GRID_PADDING, GRID_SIZE * SQUARE_LENGTH + (GRID_SIZE - 1) * WALL_SPACE + GRID_PADDING + 2, msg, GRID_BORDER_COLOR, SCREEN_BACKGROUND_COLOR);
 ...
 *
 * @param is_player_1 Whether changing for player1 or player2
...
/**
 * Draw possible movements with squares
 * @param i i index of the 7x7 matrix
 * @param j j index of the 7x7 matrix
...
void LCD_game_mode() {
    ...
    if (remote_board_status == WAITING) { GUI_Text(rect_text - 8, (MAX_Y / 2) + MENU_SPACE_BETWEEN + 13, (uint8_t *)"No second Board", White, Red); }
    else if (remote_board_status == AVAILABLE) {
        GUI_Text(rect_text + 7, (MAX_Y / 2) + MENU_SPACE_BETWEEN + 13, (uint8_t *)"Two Boards", !is_single_board ? SCREEN_BACKGROUND_COLOR : SELECTED_RECT_MENU,
                 !is_single_board ? SELECTED_RECT_MENU : SCREEN_BACKGROUND_COLOR);
...
void LCD_player_menu() {
    ...
    if (is_single_board) {
        GUI_Text(39, MENU_SIDE_PAD - 10, (uint8_t *)"Single Board: select", GRID_BORDER_COLOR, SCREEN_BACKGROUND_COLOR);
        GUI_Text(39, MENU_SIDE_PAD + 10, (uint8_t *)"the opposite player", GRID_BORDER_COLOR, SCREEN_BACKGROUND_COLOR);
    }
    else {
        GUI_Text(44, MENU_SIDE_PAD - 10, (uint8_t *)"Two Boards: select", GRID_BORDER_COLOR, SCREEN_BACKGROUND_COLOR);
        GUI_Text(73, MENU_SIDE_PAD + 10, (uint8_t *)"your player", GRID_BORDER_COLOR, SCREEN_BACKGROUND_COLOR);
    }
