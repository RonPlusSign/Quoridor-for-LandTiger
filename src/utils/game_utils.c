...
void set_game_matrix(int player, int i, int j) { game_matrix[i][j] = player; }
...
void draw_victory_msg(int player) {
    ...
    // Draw victory msg
    for (i = 1; i < 60; i++) {
        LCD_DrawLine(15, (MAX_Y - 135) / 2 + i, 225, (MAX_Y - 135) / 2 + i, White);
    } // Draw white rectangle

    if (player == 1) GUI_Text((MAX_X - 135) / 2, (MAX_Y - 120) / 2, (uint8_t *)"PLAYER 1 HAS WON!", Red, White);
    else GUI_Text((MAX_X - 135) / 2, (MAX_Y - 120) / 2, (uint8_t *)"PLAYER 2 HAS WON!", Red, White);
...
bool check_movement(int i, int j, Movement move) {
    if (move == UP) {
        if (i > 0) return (game_matrix[i - 1][j] == 0);
        else return false;
    }
    if (move == RIGHT) {
        if (j < 12) return (game_matrix[i][j + 1] == 0);
        else return false;
    }
    if (move == DOWN) {
        if (i < 12) return (game_matrix[i + 1][j] == 0);
        else return false;
    }
    if (move == LEFT) {
        if (j > 0) return (game_matrix[i][j - 1] == 0);
        else return false;
    }
    return false;
}
...
bool check_other_player(int i, int j, Movement move) {
    ...
    if (move == RIGHT) {
        if (j < 12) return (game_matrix[i][j + 2] != 0);
        else return false;
    }
    if (move == DOWN) {
        if (i < 12) return (game_matrix[i + 2][j] != 0);
        else return false;
    }
    if (move == LEFT) {
        if (j > 0) return (game_matrix[i][j - 2] != 0);
        else return false;
    }
    return false;
}
...
bool can_reach_goal_from(MatrixPoint coordinates, int i_goal) {
    ...
    if (coordinates.i == i_goal) return true;
    ...
    for (i = 0; i < 4; i++) {
        if (check_movement(coordinates_13x13.i, coordinates_13x13.j, movements[i])) {
            ...
            if (visited[adjacent_coordinates.i][adjacent_coordinates.j] == 0) {
                ...
                if (can_reach_goal_from(adjacent_coordinates, i_goal)) return true;
            }
        }
    }
...
bool is_wall_placement_possible(MatrixPoint point) {
    ...
    if (point.i < 0 || point.i >= 12 || point.j < 0 || point.j >= 12) return false;
    ...
    if (point.i % 2 == 0 && point.j % 2 == 1) { // Vertical wall
        if (game_matrix[point.i][point.j] != 0 || game_matrix[point.i + 2][point.j] != 0) return false;
    }
    else if (point.i % 2 == 1 && point.j % 2 == 0) { // Horizontal wall
        if (game_matrix[point.i][point.j] != 0 || game_matrix[point.i][point.j + 2] != 0) return false;
    }
    ...
    if (point.i % 2 == 0 && point.j % 2 == 1) { // Vertical wall
        if (game_matrix[point.i + 1][point.j - 1] != 0 && game_matrix[point.i + 1][point.j + 1] != 0) return false;
    }
    else if (point.i % 2 == 1 && point.j % 2 == 0) { // Horizontal wall
        if (game_matrix[point.i - 1][point.j + 1] != 0 && game_matrix[point.i + 1][point.j + 1] != 0) return false;
    }
...
void set_player_turn(uint8_t player) {
    ...
    if (!is_human && is_single_board && !is_player1_turn) { // Single board, player vs NPC
        ...
        if (this_board_player_id == PLAYER1) make_NPC_turn(player1_status);
        else make_NPC_turn(player2_status);
    }
}
 ...
 *
 * @param is_horizontal Whether the wall is horizontal or vertical
 ...
 */
void save_skipped_move() { __save_move(false, true, -1, -1); }


/**
 * Decode the received move from CAN
 *
 * @param move MoveMessage received from CAN
 */
void decode_received_move_from_CAN(MoveMessage move) {
    ...
    if (move.x >= 0 && move.y >= 0 && move.x <= 6 && move.y <= 6) { // Turn timer not exceeded
        if (is_moving_wall) p_13 = get_13matrix_from_universal_format(move.y, move.x, true, is_horizontal);
        else p_13 = get_13matrix_from_universal_format(move.y, move.x, false, false);
...
void update_game_matrix(uint8_t player_id, bool is_wall, MatrixPoint p) {
    ...
    if (is_wall)
        ...
        if (player_id == 1) ex_player_position = get_index_13matrix_from_7matrix(player1_status.curr_pos.i, player1_status.curr_pos.j);
        else ex_player_position = get_index_13matrix_from_7matrix(player2_status.curr_pos.i, player2_status.curr_pos.j);
...
void draw_updated_game_matrix(uint8_t player_id, bool is_wall, MatrixPoint p) {
    ...
    if (is_wall) {
        ...
        if (player_id == 1) ex_index = get_index_13matrix_from_7matrix(player1_status.curr_pos.i, player1_status.curr_pos.j);
        else ex_index = get_index_13matrix_from_7matrix(player2_status.curr_pos.i, player2_status.curr_pos.j);
